module DbTypeAst = struct
  ...
  let rec names_of_lvalue (l : lvalue) = 
    ...
    | LvAny | LvUnit -> ["any"]
    | LvValue v -> [v]
    | LvTuple vs -> List.map (fun x -> value x |> names_of_lvalue) vs |> List.fold_left (@) [] 
  ...
  let rec db_lv (nm : nm) (l : lvalue) =
    ...
    | LvTuple vs -> List.map (fun x -> value x |> db_lv nm) vs |> fun x -> DLvTuple x
  ...
  let rec db_expr (nm : nm) (e : ty typ_expr) : db_expr =
    ...
    | TTuple vs ->
      List.map (db_expr nm) vs |> fun x -> DTuple x |> typed e.typ
    | TFun f ->
      let all_names = names_of_lvalue f.lvalue.value in
      let nm = List.fold_left (fun nm n -> NameMap.add n (cnt_next ()) nm) nm all_names in
      let lets, _ = List.fold_left (fun (xs, nm) x ->
        let inner_r, nm = db_let x nm in
        (xs @ [inner_r], nm)
      ) ([], nm) f.b.lets
      in
      let expr = db_expr nm f.b.expr in
      let b = {lets; expr} in
  ...
  and db_let (l : ty typ_let_binding) nm : db_let_binding res =
    ...
    let nm = 
      match l.rec_f with
      | Rec -> List.fold_left (fun nm n -> NameMap.add n (cnt_next ()) nm) nm all_names
      | NoRec -> nm 
    in
    let lets, nm_inners = List.fold_left (fun (xs, nm) x ->
      let inner_r, nm = db_let x nm in
      (xs @ [inner_r], nm)
    ) ([], nm) l.body.lets in
    ...
    let nm = 
      match l.rec_f with
      | Rec -> nm
      | NoRec -> List.fold_left (fun nm n -> NameMap.add n (cnt_next ()) nm) nm all_names 
    in
  ...
  let db_program_of_typed_program (nm : nm) (p : ty typ_program) : db_program =
    let res, _ = List.fold_left (fun (xs, nm) x ->
