module Asm = struct
  ...
  let std_functions =
    Std.names |> SMap.to_seq |> Seq.map (fun (a, b) -> (b, a)) |> IMap.of_seq
  ...
  let map_fn_stack (fn : anf_fun_binding) =
    let empty = IMap.empty in
    let inner sign (mm, li) x =
      (IMap.add x.value (Stack li) mm, sign li (sizeof x.typ))
    in
  ...
  let alloc_closure cinfo fn args =
    ...
    let fn_name =
      match IMap.find_opt fn.value std_functions with
    ...
    let load_fn_id = mov r8 (fn.value |> ic) in
  ...
  let compile_fn cinfo (fn : anf_fun_binding) =
    ...
    let save_args =
      let rec helper lst i instrs =
        ...
        | [], _ -> instrs
        | hd :: tl, Some reg ->
            Mov (IMap.find hd.value cinfo.memory, reg) :: instrs
            |> helper tl (i + 1)
        | _, None -> instrs
      in
